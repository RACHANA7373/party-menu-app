export const menuData = [
  ...
  },
  {
    ...
    "fullDescription": "Bite-sized HTML list style caprese on skewers with extra virgin olive oil and aged balsamic reduction — light and elegant.",
    "image": "https://images.unsplash.com/photo-1775211578178-61f06027adf3?auto=format&fit=crop&w=1200&q=80",
    "ingredients": [
      {
        "name": "Cherry tomato",
...
export function filterMenuItems({ category, diet, name } = {}) {
  return menuData.filter(item => {
    if (category && category.toLowerCase() !== 'all') {
      if (item.category.toLowerCase() !== category.toLowerCase()) return false;
    }
    if (diet && diet.toLowerCase() !== 'all') {
      const isVegTarget = diet.toLowerCase() === 'veg';
      if (item.isVeg !== isVegTarget) return false;
    }
    if (name) {
...
export function getMenuItemById(id) {
  ...
  return menuData.find(item => item.id === numericId) || null;
}
